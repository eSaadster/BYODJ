...
house: 124, 909, saw bass, leadStyle piano (classic house piano stabs) or organ (garage/deep stabs), pump 0.5, swing 0.08; kick 0,4,8,12, openHat 2,6,10,14, snare 4,12. Song: 4 intro (no kick, djFilter -60) / 8 build (ramp djFilter to 0) / 16 drop / 8 breakdown (kickMute, reverb 0.5) / 16 drop / 4 outro.
...
ambient: 70, few or no drums, lydian, leadStyle flute (breathy) or bell (glassy), pad true + padVoice strings, attack 1+, release 3+, 2n, reverb 0.7; 8-16 bar sections with drifting roots (e.g. C->F->A) and reverb/cutoff ramps instead of drums.
...
2. LEAD VOICE — choose it deliberately EVERY new track. The tool REJECTS a clearFirst composition whose patterns use melodic rows but omit leadStyle (the reset would silently drop your choice), so always include it. Match the mood: piano (real grand — ballads, house/piano stabs, emotional, acoustic, "piano-like"), keys (Rhodes — lofi/jazz/neo-soul; the neutral fallback default), organ (tonewheel/garage/gospel stabs and held chords), flute (soft breathy — gentle/organic/folk), seaboard (expressive/cinematic/synthwave), bell (glassy/ambient sparkle), duo (thick detuned), pluck (koto/arp), saw (aggressive filter-swept EDM/techno ONLY). BUT if the user only asks to TWEAK an existing track (change tempo, add a section, swap a drum, "make it darker"), OMIT leadStyle entirely so their current voice is kept — sending leadStyle overwrites whatever the user hand-picked.
...
`.trim();
  ...
  var COMPOSITION_JSON_SCHEMA = {
    ...
    properties: {
      ...
      leadStyle: { type: 'string', enum: ['saw', 'pluck', 'bell', 'duo', 'seaboard', 'keys', 'piano', 'organ', 'flute'], description: 'Lead voice — pick to match the emotion. REQUIRED when clearFirst is true and patterns contain melodic rows (the tool rejects the call without it, since the reset would silently drop the choice): saw=classic filter-swept poly (waveform/envelope apply; aggressive EDM/techno only), pluck=koto/arp string, bell=glassy FM celeste/mallet (long shimmer), duo=thick detuned, seaboard=fat expressive detuned saws with bloom+vibrato (ROLI-style, best at 4n/2n with velocity swells), keys=warm Rhodes/e-piano with chorus+tremolo (lofi/jazz/neo-soul; the neutral default after a reset), piano=REAL multi-sampled acoustic grand with its own reverb (ballads/cinematic/house stabs; HTTP-only, falls back to keys on file://), organ=detuned squares with instant attack and full sustain (tonewheel/garage/gospel stabs, held chords), flute=soft breathy triangle with vibrato (gentle/organic/mellow leads). Piano and keys ignore filterCutoff by design (they route around the lead filter), so shape them with envelope/velocity/EQ instead.' },
